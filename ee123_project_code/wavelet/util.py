...
def imageStack_load(filename):
    ...
    os.system("ffmpeg -i {:s} -hide_banner -loglevel panic  {:s}frame_%2d.tiff".format(filename, path))
...
def imageStack_save(stack, path, framerate):
    os.system("rm -rf {:s}".format(path))
    os.system("mkdir -p {:s}".format(path))
...
def GIF_save(path, framerate):
    os.system("ffmpeg -r {:d} -i {:s}/frame_%2d.tiff -hide_banner -loglevel panic -compression_level 0 -plays 0 -f apng {:s}/animation.png".format(framerate, path,path))
